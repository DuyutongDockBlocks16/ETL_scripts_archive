...
use dmc_dev;
drop table dmc_dev.znyjjf_zh;
create table dmc_dev.znyjjf_zh as
select 
	type
	,merchant_no---------------------------------自助收银
	from 
	(
	select v021 as type---业务类型
	,v031 as merchant_no-----商户号
	,v036 as deviceid---设备id
	from odm.ODM_INN_DSP_BIG_00_000_I_D ------下游idm层表：idm_f02_pay_sfjh_tx_dtl_i_d
	where v021='selfhelp'
	and dt<=default.sysdate(-1)
	and v002='2'
	group by 1,2,3
	) a
	left join
	...
	(
		...
		,cooperate_model
		,bind_status
		,row_number() over(partition by device_no order by modified_date desc) as rndev
		from odm.odm_pay_jhzf_n_self_device_info_i_d) e 
		------------杜昱彤：这个表的idm层任务下线了
		where e.rndev='1') b
		on a.deviceid=b.device_no
	where b.cooperate_model <> 'test'
...
union ALL--------------------------------------------------------------pos，六里桥pad
	...
	,v031 as merchant_no-----商户号
	from odm.ODM_INN_DSP_BIG_00_000_I_D  ------下游idm层表：idm_f02_pay_sfjh_tx_dtl_i_d
	where v021 in('pos','phenixPay')----pos，phenixpay 六里桥pad
	and dt<=default.sysdate(-1) and v002='2'
	group by 1,2
...
------第二段，同一类型内商户号去重
	use dmc_dev;
	drop table dmc_dev.znyjordjf;
	create table dmc_dev.znyjordjf as
	select *
	from
	(select type,merchant_no
	,row_number() over(partition by merchant_no order by type) as rn
	from dmc_dev.znyjjf_zh) a
	where rn=1

-----第三段：匹配各商户号的交易情况
	use dmc_dev;
	drop table dmc_dev.znyjmerjf;
	create table dmc_dev.znyjmerjf as
...
------第四段：统计各产品的订单、交易额、毛利
	...
	from dmc_dev.znyjmerjf
	where day between '2021-04-09' and '2021-04-15'---根据自己的需要卡时间就好
	group by 1,2
