...
----------收单业务
 drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_01 ;
 create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_01
 as       
 select 
...
-----------------------③分类为“代付”，交易名称为“结算后出款”，计提银行成本为“0”的部分。
 ----------------------代付业务
insert into table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_01
select 
    acc_date                                                                                as dim_day_id 
   ,mer_id         
   ,trade_type      
...
----------全部业务
 drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_02 ;
 create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_02
 as  
select 
    dim_day_id
   ,mer_id          
   ...
   ,sys_src 
  ,card_type   
from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_01
where pay_tools<>'白条'
union all
select 
    dim_day_id
...
--③交易名称为“代付至银行卡-退票（新）（D104）”、“交易撤销（U100）”、“退款受理”（W100）的订单金额为负数，正退为“退单”，其余订单金额取绝对值，正退为“正单”。（可用来校验二-2-①） 经沟通这条废弃
 drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_03 ;
 create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_03
 as  
select 
    dim_day_id
   ,mer_id           
   ...
   ,sys_src
   ,card_type   
from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_02
union all 
select 
    dim_day_id
   ,mer_id           
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_04 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_04
as  
...
--业务归属 “体系内”、业务大类 “支付集团业务”、业务小类“线上C端”中。
 drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_05 ;
 create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_05
 as  
select
    dim_day_id 
   ,mer_id          
   ...
   ,sys_src
   ,card_type   
from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_03 p1
left join dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_04 p2 on p1.dim_day_id=p2.days
union all
select
    dim_day_id 
   ,mer_id          
   ...
   ,sys_src 
   ,card_type   
from (select *from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_03 
where dept_nm ='支付业务部' 
  and mer_ownr='体系外' 
...
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_06 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_06
as 
select
    acc_date                                                                                         as dim_day_id 
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_07 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_07
as 
--二、收入调整
...
select
   ...
   ,sys_src
   ,card_type   
from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_05
where dept_nm='支付业务部' 
  and mer_ownr='体系内'
  and pay_tools='小金库'
union all
...
$SQL_BUFF[8]=qq(
set mapred.job.name=dmfbc_oar_bc_pay_sis_fin_rpt_i_d_dyt_08;
use dmf_tmp;
---------------------封闭卡数据
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_dyt_08;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_dyt_08 as 
SELECT  t1.created_date as dim_day_id,
        ...
        FROM   (SELECT substr(created_date,1,10) as created_date,
                ...
                join 
                (SELECT Substr(created_date,1,10) as created_date, batch_no, term_no, store_no, merchant_accept
                FROM odm.ODM_PAY_SETTLEMENT_FLOW_013_I_D   --结算流水表
                where status > 0 
                and Substr(created_date,1,10) >='$TX_10B_DATE'
                and substr(created_date,1,10)<='$TX_PREV_DATE'
                group by Substr(created_date,1,10), batch_no, term_no, store_no, merchant_accept
                ) t2
...
use dmf_tmp;
---------------------封闭卡数据
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_08;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_08 as 
SELECT  dim_day_id,
        merchant_accept, 
...
from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_dyt_08 t
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_09;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_09 
as 
select 
...
from (select * from odm.ODM_PAY_MERCHANT_BASE_013_S_D where dt=date_sub('$TX_DATE',1)) a    --商户基础信息
 join dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_08 d
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_pos_my2_tradeorder;
create table dmf_tmp.dmf_tmp_pos_my2_tradeorder as 
select * from (
...
;
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_pos_my2_paymentresult;
create table dmf_tmp.dmf_tmp_pos_my2_paymentresult as 
select * from (
...
drop table if exists dmf_tmp.dmf_tmp_pos_pos_fin_list;
create table dmf_tmp.dmf_tmp_pos_pos_fin_list as
select 
    '建行' as bankname,
    '正向交易' as s_type,
    to_date(o.createtime) as dim_day_id,
    sum(o.trxamount) as amt
from dmf_tmp.dmf_tmp_pos_my2_tradeorder o 
join dmf_tmp.dmf_tmp_pos_my2_paymentresult pr on pr.payId = o.payId
where 
...
insert into table dmf_tmp.dmf_tmp_pos_pos_fin_list
select 
    '农行' as bankname,
    '正向交易' as s_type,
    to_date(o.createtime) as dim_day_id,
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_10;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_10 
as 
select      
    dim_day_id                                      as dim_day_id
   ,cast(null as string)                            as mer_id          
   ...
   ,'调整项'                                        as sys_src 
   ,cast(null as string)                            as card_type   
from  dmf_tmp.dmf_tmp_pos_pos_fin_list
group by dim_day_id
;
)
;
...
select      
   ...
   ,case when fir_lvl_name in ('TO C-用户运营','利息收入','利息收入-京东生态','利息收入-京东生态-金融行业','利息收入-金融科技群') then fir_lvl_name 
         when fir_lvl_name in ('支付集团业务-B端','支付商城业务-B端') then '支付集团业务'
        end                      as mer_big_type    
   ,case when fir_lvl_name in ('TO C-用户运营','利息收入','利息收入-京东生态','利息收入-京东生态-金融行业','利息收入-金融科技群') then fir_lvl_name 
         when fir_lvl_name in ('支付集团业务-B端','支付商城业务-B端') then '线上B端'
        end                         as mer_sma_type
   ,case when fir_lvl_name in ('TO C-用户运营','利息收入','利息收入-京东生态','利息收入-京东生态-金融行业','利息收入-金融科技群') then '体系外' 
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_12 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_12
as 
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_05                     --主体业务
union all                                                                                                                                             
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_06                     --白条还款
union all                                                                                                                                             
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_07                     --调减收入
union all                                                                                                                                            
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_09                     --封闭卡
union all                                                                                                                                            
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_10                     --调整项POS
union all                                                                                                                                           
select date(dim_day_id) as dim_day_id,mer_id,trade_type,trade_name,product_id,pay_tools,mer_date,acc_date,order_amt,trade_amt,cast(mer_fee as double),cast(bank_fee as double),cast(trade_num as integer),flag,bus_flag,dept_nm,mer_big_type,mer_sma_type,mer_ownr,prod_type,sys_src,card_type from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_11                     --线下导入数据 POP结算、TO-C运营、利息收入
;
)
;
$SQL_BUFF[14]=qq(
...
use dmf_tmp;
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_13 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_13
as
select 
   dim_day_id                                  --统计日期维度
  ,mer_id                                      --二级商户号id
  ...
  ,dim_day_id                 as dt
from 
dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_12
where 1=1
...
-----------------------------把通道成本的作业内容融合到该作业当中
------增加鹏华的通道成本费用
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_14 ;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_14
as
select 
...
use dmf_tmp;
------拆分消费金融 白条、金条还款额占比
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_15;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_15
as 
--白条还款表
select 
...
use dmf_tmp;
------拆分消费金融 白条、金条还款额占比
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_151;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_151
as 
select 
     tt.repay_date 
 ...
 from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_15 tt
...
use dmf_tmp;
------拆分消费金融 白条、金条还款额占比
drop table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_16;
create table dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_16
as 
select 
   dim_day_id                                  --统计日期维度
  ,mer_id                                      --二级商户号id
  ...
  ,dt                                          --时间分区  
from 
dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_13
...
select 
  ...
  ,dt                                               --时间分区  
from 
(select *from dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_13
where mer_sma_type='白条还款') t1
left join dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_151 t2 
...
use dmf_bc;
insert overwrite table dmf_bc.dmfbc_oar_bc_pay_sis_fin_rpt_i_d partition (dt)
select 
   dim_day_id                                  --统计日期维度
  ,mer_id                                      --二级商户号id
  ,trade_type                                  --清结算系统交易类型
  ...
  ,dt
from 
dmf_tmp.dmf_tmp_dmfbc_oar_bc_pay_sis_fin_rpt_i_d_16
group by 
 dim_day_id  
,mer_id      
,trade_type  
