    ...
    *,
    ...
    use dmf_tmp;
     ...
     as
        ...
        case when (voucher.aux_subject_1_type='3' or voucher.aux_subject_1_type='2#2' or voucher.aux_subject_1_type='MASTER_CUSTOMER') then trim(aux_subject_1)---T0和手工凭证用的是'MASTER_CUSTOMER'，T1用的是'3''2#2'，这个逻辑来源于肖佳
             when (voucher.aux_subject_2_type='3' or voucher.aux_subject_2_type='2#2' or voucher.aux_subject_2_type='MASTER_CUSTOMER') then trim(aux_subject_2)
             when (voucher.aux_subject_3_type='3' or voucher.aux_subject_3_type='2#2' or voucher.aux_subject_3_type='MASTER_CUSTOMER') then trim(aux_subject_3)
        else '' end                         as merchant_id   , --客商id,取辅助核算3和2#2和'MASTER_CUSTOMER'这俩都是客商的意思
        ...
        from 
        (select * from dmf_tmp.tmp_cwmart_month_evidence_detail_s where to_date(update_time)<='{TX_DATE}') voucher--算哪天要哪天之前的数据
        inner join 
        (
    ...
    where 1=1
    and yn_flag='1' 
    and dt ='{TX_DATE}'  --主数据的历史dt脏数据太多了，重跑的时候用最新版本的映射
    )mercode
    ...
    voucher.merchant_id             as merchant_id          ---客商id
    ...
    coalesce(voucher.ebs_company_code,preperiod.company_code)    as company_code,
    ''                          as company_name,
    coalesce(voucher.base_currency_code,preperiod.base_currency_code)  as base_currency_code,
    coalesce(voucher.currency_brief_code,preperiod.currency) as currency,
    coalesce(voucher.subject_no,preperiod.subject_code)      as subject_code,
    ''                          as subject_name,
    coalesce(voucher.merchant_code,preperiod.merchant_code)  as merchant_code,
    ''                          as merchant_name,
    coalesce(voucher.ebs_biz_code,preperiod.bizline_code)    as bizline_code,
    ''                          as bizline_name,
    coalesce(voucher.ebs_proj_code,preperiod.project_code)   as project_code,
    ''                          as project_name,
    ...
    coalesce(voucher.merchant_id,preperiod.merchant_id)      as merchant_id ---客商id
    ...
    on  coalesce(voucher.ebs_company_code,'')=coalesce(preperiod.company_code,'')
    and coalesce(voucher.subject_no,'')=coalesce(preperiod.subject_code,'')
    and coalesce(voucher.ebs_biz_code,'')=coalesce(preperiod.bizline_code,'')
    and coalesce(voucher.ebs_proj_code,'')=coalesce(preperiod.project_code,'')
    and coalesce(voucher.currency_brief_code,'')=coalesce(preperiod.currency,'')
    and coalesce(voucher.base_currency_code,'')=coalesce(preperiod.base_currency_code,'')
    and coalesce(voucher.merchant_code,'')=coalesce(preperiod.merchant_code,'')   
    ) res
    where 1=1
    and (  -----把都是指标都是0的联合主键过滤掉，防止数据无限膨胀
