...
"sql_01": """
set mapred.job.name=job_sdm_zjxt_zfyw_sfkmsr_transaction_i_01;
set mapred.max.split.size=64000000;
set hive.stats.column.autogather=false;
use dmf_gj;

alter table dmf_gj.dmfgj_gj_pay_fi_hs_transaction_detail_i_d drop partition(dt = '{TX_DATE}' ,data_source_em='a07_zfyw_sfkmsr');
alter table dmf_gj.dmfgj_gj_pay_fi_hs_transaction_detail_i_d add partition(dt = '{TX_DATE}' ,data_source_em='a07_zfyw_sfkmsr');
""",

"sql_02": """
set mapred.job.name=job_sdm_zjxt_zfyw_sfkmsr_transaction_i_02;
set mapred.max.split.size=64000000;
set hive.stats.column.autogather=false;
use dmf_tmp;

drop table dmf_tmp.tmp_sdm_zjxt_zfyw_sfkmsr_sdm_gj_transaction_i;
create table dmf_tmp.tmp_sdm_zjxt_zfyw_sfkmsr_sdm_gj_transaction_i
as
select
CONCAT_WS('_', new_source_table, 'a07_zfyw_sfkmsr', trans_type, writeoff_status,joinkey,'{TX_DATE}') AS origin_id,
'TNR001' as serial_no, --????????????????????????????????????????????????????????????
dmf_bc.dmdictdesc('BUSI_TYPE_3_1','303500','{TXDATE}') as biz_type,  --把产品的节点id
dmf_bc.dmdictdesc('BUSI_TYPE_3_2','303500','{TXDATE}') as biz_line,  --????????????????????????????????????????????????????????????
'303500' as product_no,   --????????????????????????????????????????????????????????????
'0' as trans_type,
'1003268' as company_no1,--查客商主数据的客商id，记住是客商id
'' as company_no2,
'' as company_no3,
'' as company_no4,
'1014198' as company_no5, --客商-个人
'6852' as borrow_bank_acct,
'' as loan_bank_acct,
...
from (
    select *
        ,'dmfbc_gj_pay_sfkmsr_transaction_i_d' as new_source_table
        ,dmf_bc.dmdictdesc('zjxt_zfyw_sfkmsr_trans_type',bankcode,batchid,txtype)  as trans_type --交易环节，这个要等配好之后
        , case when data_type = '1' then 'NORMAL' else 'WRITEOFF' end as writeoff_status
from dmf_bc.dmfbc_gj_pay_sfkmsr_transaction_i_d a1
where  dt = '{TX_DATE}'
...
"sql_03": """
set mapred.job.name=job_sdm_zjxt_zfyw_sfkmsr_transaction_i_03;
set mapred.max.split.size=64000000;
set hive.stats.column.autogather=false;
use dmf_gj;

INSERT OVERWRITE TABLE dmf_gj.dmfgj_gj_pay_fi_hs_transaction_detail_i_d PARTITION (dt = '{TX_DATE}' ,data_source_em='a07_zfyw_sfkmsr')
SELECT
origin_id,
upper(concat_ws('_',serial_no, '{TX_DATE}', 'a07_zfyw_sfkmsr', default.getmd5(origin_id))) as serial_no,
biz_type,
biz_line,
product_no,
trans_type,
...
FROM dmf_tmp.tmp_sdm_zjxt_zfyw_sfkmsr_sdm_gj_transaction_i
