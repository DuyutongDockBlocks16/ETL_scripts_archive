...
sql_map={
    ...
    *
    ...
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000035_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000036_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000037_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000038_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000039_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000041_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    union all
    select
    *
    from odm.odm_fi_tz_dm_gj_general_g_journal_voucher_sync_000042_month_i_i_d 
    where kd_voucher_status ='SUCCESS'---取EBS回传状态为真的数据 
    and status='0'---只取有效的凭证    
    and manual_dt='{TXPREMONTH}'--只取记账时间为上月的数据
    ;
    ...
    from (select * from dmf_tmp.tmp_cwmart_month_evidence_personmerchant_detail_s where dt='{TX_DATA_DATE}')aaa   
    left join
    (
    ...
    "sql_07": """
    use dmf_tmp;
    alter table dmf_tmp.tmp_cwmart_month_evidence_personmerchant_detail_003_s drop if exists partition(dt = '{TX_DATA_DATE}')
    ;
    """,
    
    "sql_08": """
    use dmf_tmp;
    insert into dmf_tmp.tmp_cwmart_month_evidence_personmerchant_detail_003_s partition(dt = '{TX_DATA_DATE}')
    select
    '{TXPREMONTH}'                  as periodname,          ---期间
    ...
    from (select * from dmf_tmp.tmp_cwmart_month_evidence_personmerchant_detail_002_s where dt='{TX_DATA_DATE}' ) voucher
    left join 
    (
        select 
    ...
    "sql_09": """
    use dmf_tmp;
    alter table dmf_tmp.tmp_cwmart_month_evidence_personmerchant_sum drop if exists partition(dt = '{TX_DATA_DATE}')
    ;
    """,
    ...
    "sql_10": """
    use dmf_tmp;
    insert into dmf_tmp.tmp_cwmart_month_evidence_personmerchant_sum partition(dt = '{TX_DATA_DATE}')
    select 
    periodname,
    ebs_company_code,
    subject_no,
    ...
    from (select * from dmf_tmp.tmp_cwmart_month_evidence_personmerchant_detail_003_s where dt='{TX_DATA_DATE}') aaa
    group by 
    periodname,
    ebs_company_code,
    subject_no,
    ...
    "sql_11": """
    use dmf_tmp;
    alter table dmf_tmp.tmp_cwmart_month_evidence_personmerchant_sum_002 drop if exists partition(dt = '{TX_DATA_DATE}')
    ;
    """,    
    
    "sql_12": """
    use dmf_tmp;---抽所有集市业务的月大凭证，最后unionall起来
    insert into dmf_tmp.tmp_cwmart_month_evidence_personmerchant_sum_002 partition(dt = '{TX_DATA_DATE}')
    select
    '{TXPREMONTH}' as periodname,
    voucher.ebs_company_code,
    voucher.subject_no,
    voucher.ebs_biz_code,
    voucher.ebs_proj_code,
    voucher.currency_brief_code,
    voucher.base_currency_code,
    voucher.merchant_code,
    coalesce(preperiod.end_balance,0.0)      as begin_balance,--期初余额（原币）上期期末当本期期初
    ...
    FULL OUTER JOIN (
        select * from dmf_rpt.dmfrpt_eas_sf_pers_subj_bal_tab_amt_i_m where 
        period_name=substr('{TXPREMONTH}'-1,0,6) ---取上个期间的期末，当这个期间的期初
        ) preperiod
