...
(
    SELECT
        to_date(bank_rec_date) as data_time,
        m.pay_currency_type as rec_currency_type,
        sum(m.pay_amount) as rec_amount
    FROM
    (
        select
            *
    ...
    group by pay_currency_type, to_date(bank_rec_date)
    union all
    SELECT
        to_date(bank_rec_date) as data_time,
        k.rec_currency_type,
    ...
    where rn = 1
        ...
        and to_date(bank_rec_date) >= date_add('$TX_DATE', -39)
        and to_date(bank_rec_date) <= '$TX_DATE'
    group by rec_currency_type, to_date(bank_rec_date)
    union all
    select
        to_date(data_time) as data_time,
        rec_currency_type,
        rec_amount
