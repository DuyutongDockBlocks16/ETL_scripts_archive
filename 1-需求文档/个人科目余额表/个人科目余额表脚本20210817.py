    ...
    use dmf_tmp;
     ...
     as
        ...
        case when (voucher.aux_subject_1_type='3' or voucher.aux_subject_1_type='2#2' or voucher.aux_subject_1_type='MASTER_CUSTOMER') then trim(aux_subject_1)---T0和手工凭证用的是'MASTER_CUSTOMER'，T1用的是'3''2#2'，这个逻辑来源于肖佳
             when (voucher.aux_subject_2_type='3' or voucher.aux_subject_1_type='2#2' or voucher.aux_subject_1_type='MASTER_CUSTOMER') then trim(aux_subject_2)
             when (voucher.aux_subject_3_type='3' or voucher.aux_subject_1_type='2#2' or voucher.aux_subject_1_type='MASTER_CUSTOMER') then trim(aux_subject_3)
        ...
        from 
        (select * from dmf_tmp.tmp_cwmart_month_evidence_detail_s where date_add(to_date(etl_dt),-1)<='{TX_DATE}') voucher--算哪天要哪天之前的数据
        inner join 
        (
