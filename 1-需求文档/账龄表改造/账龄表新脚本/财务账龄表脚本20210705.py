...
set merge.mapredfiles=true;                                                                          
set merge.smallfiles.avgsize=512000000; 
set merge.size.per.task=1024000000;
...
create table if not exists dmf_tmp.ebs_bp_off_cwaduit_coas_age_tmp
    ...
    select
        ...
        from (
          ...
          left join 
          (
              select coalesce(providerfid,'') as providerfid,max(coalesce(providername,'')) as providername from (select * from dmf_tmp.ebs_bp_cwaduit_coas_acct_period_evidence_d_s where dt = '{TX_DATA_DATE}'and  providernum is not null ) yyy group by providerfid
