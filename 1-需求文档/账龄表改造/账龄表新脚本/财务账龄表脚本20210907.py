...
stored AS orc;
    ...
    coalesce(merchant.vendor_site_code,main.vendor_siet_code)   as providerfid	,	--	供应商fid,因为要用客商fid作为汇总的维度，但ebs中没有公司id字段只好选用公司编码
    coalesce(merchant.vendor_site_code,main.vendor_siet_code)   as providernum	,	--	供应商编码
    coalesce(merchant.company_name,main.vendor_name)            as providername	,	--	供应商
    ...
    left join (
        select ou,vendor_site_code,company_name from odm.odm_fi_fin_subject_merchant_s_d 
        where 1=1 and yn_flag='1' and status = '1'and dt  = default.sysdate(-1)
    ) merchant
    on main.coa_ic_code = merchant.ou
