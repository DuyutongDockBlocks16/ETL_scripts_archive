...
sql_map={
    ...
    "sql_12": """
    use dmf_tmp;
    alter table dmf_rpt.dmfrpt_eas_sf_ebs_biz_line_plus_i_d drop if exists partition(dt = '{TX_DATE}')
    ;
    """,    
    
    "sql_13": """
    use dmf_tmp;
    insert into table dmf_rpt.dmfrpt_eas_sf_ebs_biz_line_plus_i_d partition(dt = '{TX_DATE}')
    select  
    etl_dt	,
    rt_meta	,
    id	,
    ...
    source_end_date	
    from
    odm.odm_fi_ebs_business_line_i_d
    where 1=1
    and substr(end_dt,1,10)>'{TX_DATE}'
    and substr(source_end_date,1,10)>'{TX_DATE}'
    ;
    """,    
