...
$SQL_BUFF[14]=qq(
        ...
        voucher_date,
        business_date
        from dmf_bc.dmfbc_mrpt_pay_fi_biz_line_mntr_i_d----JCW_DMFBC_MRPT_PAY_FI_PRD_DTL_CUS_ZFGB_I_D
        where dt='$TX_DATE'
        group by 
        subject_manage_code,
        subject_manage_name,
...
$SQL_BUFF[15]=qq(
        ...
        null,
        null,
        push.business_manage_code as business_account_code,---核算业务线编码（格式：YWX.992.08.06）这里要改一下映射，
        push.business_manage_name as business_account_name,
        null,----department_account_code对的这个字段是空的
        null,
        3,
        null,
        null,
        ...
        else account.supplier_code end ,---得看下这里怎么改，做要一下，用主数据做个映射
        push.supplier_name,---得看下这里怎么改，直接用就行
        null,
        null,
        null,
        null,
        ...
        left join  (select * from (select *,row_number() over(partition by supplier_name order by id) rn 
        from odm.odm_fi_eas_daily_details_account_s_d where dt='$TX_DATE') a 
          where a.rn=1
      )  account
