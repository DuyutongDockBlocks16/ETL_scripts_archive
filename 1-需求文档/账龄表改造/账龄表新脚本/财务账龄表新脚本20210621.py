...
stored AS orc;
    ...
    ''                                                          as accviewlongnum	,	--	会计科目
    coa_acc_code                                                as accviewnum	,	--	科目编码
    ''                                                          as accviewfdc	,	--	科目方向
    ...
    vendor_siet_code                                            as providerfid	,	--	供应商fid,因为要用客商fid作为汇总的维度，但ebs中没有公司id字段只好选用公司编码
    vendor_siet_code                                            as providernum	,	--	供应商编码
    vendor_name                                                 as providername	,	--	供应商
    ...
    ''                                                          as remark	,	--	备注
    ''                                                          as userid		--	报表创建者id
    from odm.ODM_FI_EBS_CUX_JDTMAS_JOURNAL_DETAILS_V_I_D --ebs凭证明细表
...
create table if not exists dmf_tmp.ebs_bp_cwaduit_coas_age_tmp
    ...
    select
        ...
        from (
          ...
          left join 
          (
              select coalesce(providerfid,'') as providerfid,max(coalesce(providername,'')) as providername from (select * from dmf_tmp.ebs_bp_cwaduit_coas_acct_period_evidence_d_s where dt = '{TX_DATA_DATE}') yyy group by providerfid
      
          ) c on a.providerfid = c.providerfid
          left join 
          (
             select 
              case when ( trim(bizlinenum)=''  or bizlinenum is  null) then ''  else bizlinenum   end as bizlinenum ---有空的数据，还有空字符串的数据，需要这样处理一下
              ,max(
                case   when ( trim(bizlinenum)<>''  and bizlinenum is not null) then coalesce(bizline, '')
                else ''
                end
                ) as bizline 
              from (select * from dmf_tmp.ebs_bp_cwaduit_coas_acct_period_evidence_d_s where dt = '{TX_DATA_DATE}') zzz
              group by  case when ( trim(bizlinenum)=''  or bizlinenum is  null) then ''  else bizlinenum   end
    ...
    select
        ...
        from (
          ...
          (
              ...
              from (select * from dmf_tmp.ebs_bp_cwaduit_coas_acct_period_evidence_d_s where dt = '{TX_DATA_DATE}') zzz group by bizlinenum
          ) d on a.bizlinenum = d.bizlinenum   
          left join
          (
             select accviewlongnum,accviewnum from 
