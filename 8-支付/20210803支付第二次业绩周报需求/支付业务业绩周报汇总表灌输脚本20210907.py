...
''					        as prd_thd_code,
''					        as prd_thd_name,
main.bizline_code			as man_line_code,
...
mgmtdept.l5_code            as ind_scd_code,
mgmtdept.l5_name            as ind_scd_name,
mgmtdept.l6_code            as ind_thd_code,
mgmtdept.l6_name            as ind_thd_name,
...
on main.bizline_code       = manaline.code    --产品维表（可用管理业务线关联）
left join (SELECT * from dmf_bi.dmfbi_profits_dim_dept_mgmt_s_d 	      WHERE dt = '{TX_DATE}') mgmtdept 
...
on main.index_code         = mdmsub.sub_code--管理科目维表
...
''					        as prd_thd_code,
''					        as prd_thd_name,
main.manage_bizline_code	as man_line_code,
manaline.name               as man_line_name,
''                          as remark1,
...
left join (SELECT * from dmf_bi.dmfbi_profits_fi_manage_line_code_dim_s_d WHERE dt = '{TX_DATE}') manaline 
on main.manage_bizline_code = manaline.code    --产品维表（可用管理业务线关联）
left join (SELECT * from dmf_bi.dmfbi_profits_dim_dept_mgmt_s_d 	      WHERE dt = '{TX_DATE}') mgmtdept 
on main.bizdept			   = mgmtdept.code	  --行业维表   
left join (SELECT * from dmf_dim.dmfdim_dim_profits_mdm_mag_sub_s_d 	  WHERE dt = '{TX_DATE}') mdmsub   
on main.manage_subject_code = mdmsub.sub_code--管理科目维表
