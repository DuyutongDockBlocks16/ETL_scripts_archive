...
UNION ALL-------------------------------------------------------------双屏
	...
	select * 
	from
	(select case when prod_type in ('双屏收银机及配件','双屏收银机及配件-新通路渠道') then 'xintonglu' ----双屏_新通路渠道
            when prod_type='双屏收银机及配件-京东数码渠道'  then 'JDSM'----双屏_京东数码渠道
       end as type---业务类型
    ...
    "sql_04": """
    use dmf_tmp;
    drop table if exists dmf_tmp.dmcbc_bc_znyjordjf_sum_001 ;
    ;
    """,
    
    "sql_05": """
    use dmf_tmp;
    create table if not exists dmf_tmp.dmcbc_bc_znyjordjf_sum_001
    as 
    select 
	type,
	day,
    ...
    where day <= date_sub('{TX_DATE}',3) ---后三天的费率不准，所以单独做计算
	group by 1,2
    ;
    """,
    ...
    "sql_06": """
    use dmf_tmp;
    drop table if exists dmf_tmp.dmcbc_bc_znyjordjf_eval_rate ;
    ;
    """,
    
    "sql_07": """
    use dmf_tmp;
    create table if not exists dmf_tmp.dmcbc_bc_znyjordjf_eval_rate
    as  
    select
    type,
    maoli/GMV as eval_rate
    from dmf_tmp.dmcbc_bc_znyjordjf_sum_001
    ...
    "sql_08": """
    use dmf_tmp;
    drop table if exists dmf_tmp.dmcbc_bc_znyjordjf_sum_002 ;
    ;
    """,
    
    "sql_09": """
    use dmf_tmp;
    create table if not exists dmf_tmp.dmcbc_bc_znyjordjf_sum_002
    as  
    select
    main.type,
	main.day
	,main.ord----聚合订单数
	,main.GMV---聚合交易额
	,main.GMV*rate.eval_rate as maoli---收入(含税)
    ,'预估数据'
    ,''
    ,''
    ,''
    ...
    left join ( select type,eval_rate from dmf_tmp.dmcbc_bc_znyjordjf_eval_rate ) rate on main.type = rate.type
    ;
    """,
    
    "sql_96": """
    use dmf_bi;
    alter table dmf_bi.dmfbi_profits_pay_equip_inds_week_s_d drop if exists partition(dt = '{TX_DATE}')
    ;
    """,   
    
    "sql_97": """
    use dmf_bi;
    insert into table dmf_bi.dmfbi_profits_pay_equip_inds_week_s_d partition(dt = '{TX_DATE}')--用这个表做存档
    
    select 
    *
	from dmf_tmp.dmcbc_bc_znyjordjf_sum_001
    union all---最后把真实数据和预估数据结合到一起
    select 
    *
	from dmf_tmp.dmcbc_bc_znyjordjf_sum_002
    ...
    *
    ...
    "sql_99": """
    use dmf_bi;
    insert overwrite table dmf_bi.dmfbi_profits_pay_equip_mer_id_a_d
    select 
	type,
    merchant_no,
    rn
	from dmf_tmp.dmcbc_bc_znyjordjf
    ;
    """,
}
