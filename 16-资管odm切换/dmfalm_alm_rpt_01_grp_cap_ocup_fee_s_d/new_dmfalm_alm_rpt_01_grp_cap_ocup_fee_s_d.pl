...
SELECT
 ...
 FROM
        ...
        FROM
            odm.ODM_AM_ABS_INVESTOR_LOAN_0000_S_D  B ----odm.ODM_FI_INVESTOR_LOAN_S_D  B
        WHERE
            B.DT='$TX_DATE'
            ...
            AND  b.INVESTOR_ID   IN (
               SELECT
                        investor_id
                    FROM
                        odm.ODM_AM_ABS_PLAT_INVESTOR_S_D ----odm.ODM_FI_PLAT_INVESTOR_S_D
                    WHERE
                        DT='$TX_DATE'
                        AND investor_type='0'
                        AND  PLAT_id IN (
...
SELECT
    ...
    FROM
        (     SELECT
            SUM(LOAn_amount)     AS   loan_amount
        FROM
            odm.ODM_AM_ABS_INVESTOR_LOAN_0000_S_D k2----odm.ODM_FI_INVESTOR_LOAN_S_D k2
        WHERE
            DT = '$TX_DATE'
    ...
    LEFT JOIN
        (
            SELECT
                ...
                END)    AS UnderPrincipal
            FROM
                odm.ODM_AM_ABS_INVESTOR_FLOW_0000_S_D k1-----odm.ODM_FI_INVESTOR_FLOW_S_D k1
            where
                dt = '$TX_DATE'
                AND TO_DATE(
                    create_time
